...
function update_process_tracking() {
    ...
    if [ -z "$pmon_output" ]; then
        ...
        while IFS='|' read -r _ content _; do
            ...
            # - PID is validated to be numeric only (line 209)
            # - Memory is validated to be numeric only (line 215)
            # - Process name has single quotes escaped (line 222)
            # - Timestamps are generated by this script, not user input
...
SQL
    ...
    while read -r gpu_id pid ptype sm mem enc dec command rest; do
        ...
        # Insert snapshot and update tracking
        # NOTE: SQL injection is prevented by:
        # - PID is validated to be numeric only (line 290)
        # - Memory is validated to be numeric only (line 296)
        # - Process name has single quotes escaped (line 303)
        # - Timestamps are generated by this script, not user input
        if sql_result=$(sqlite3 "$DB_FILE" 2>&1 <<SQL
INSERT INTO process_snapshots (timestamp_epoch, pid, process_name, memory_usage)
